<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Document</title>
  <!-- CDN -->
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">

  <!-- FONTS -->
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Sarabun:wght@400;500;600;700&display=swap" rel="stylesheet">
  <!-- CUSTOM CSS -->
  <link rel="stylesheet" href="./assets/style/main.css" />
  <link rel="stylesheet" href="./assets/style/product.css" />

</head>

<body>
  <?php include 'main-menu.php'; ?>
  <main>
    <section id="header" class="heading-box">
      <div class="container">
        <div class="row">
          <div class="col-md-8 heading-main-box">
            <h1 class="title-heading fw-700 text-white-color mb-4">
            Connect Physical And Digital World Together
            </h1>
          </div>

        </div>
      </div>
    </section>

    <section class="all-products" id="home-product">
      <div class="container">
        <div class="descript-product">
          <div class="text-center">
            <h2 class="descript-product-title fw-700 mt-4">Product</h3>
              <p class="descript font-size-18 fw-400 mx-auto">Choose from our cutting-edge products for a better society</p>

          </div>
          <!-- <div class="categories-product">
            <h4 class="categories-product-title mb-3 fw-500">Product Categories</h4>
            <div class="row flex-column">
              <a href="product-child.php" class="dark-grey-color">
                <div class="product-category-items d-flex">
                  <div class="card mb-3">
                    <div class="d-flex align-items-center">
                      <div class="bg-light-grey">
                        <img src="./assets/images/detail-product1.svg" alt="ALVIS Traffic Camera" class="card-img img-left">
                      </div>
                      <div class="card-body">
                        <p class="card-title product-category-title fw-500 font-size-18 pl-1">AI Camera</p>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
              <a href="AI-box.php" class="dark-grey-color">
                <div class="product-category-items d-flex">
                  <div class="card mb-3">
                    <div class="d-flex align-items-center">
                      <div class="bg-light-grey">
                        <img src="./assets/images/Smart-Retail-Box.svg" alt="ALVIS Traffic Camera" class="card-img img-left">
                      </div>
                      <div class="card-body">
                        <p class="card-title product-category-title fw-500 font-size-18 pl-1">AI Box</p>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </div>
          </div> -->
          <div class="categories-product mb-5">
            <h2 class="categories-product-title mb-3 fw-600" style="margin-left: 28%;">Product Categories</h2>
            <div class="row mb-5 pb-4">
              <div class="col-md-12">
                <a href="product-child.php" class="dark-grey-color">
                  <div class="product-category-items d-flex">
                    <div class="card mb-3 mx-auto">
                      <div class="d-flex align-items-center">
                        <div class="bg-light-grey">
                          <img src="./assets/images/detail-product1.svg" alt="ALVIS Traffic Camera" class="card-img img-left">
                        </div>
                        <div class="card-body">
                          <p class="card-title product-category-title fw-500 font-size-18 pl-1">AI Camera</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </a>
              </div>
              <div class="col-md-12">
                <a href="AI-box.php" class="dark-grey-color">
                  <div class="product-category-items d-flex">
                    <div class="card mb-3 mx-auto">
                      <div class="d-flex align-items-center">
                        <div class="bg-light-grey">
                          <img src="./assets/images/Smart-Retail-Box.svg" alt="ALVIS Enforcement Camera" class="card-img img-left">
                        </div>
                        <div class="card-body">
                          <p class="card-title product-category-title fw-500 font-size-18 pl-1">AI Box</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </a>
              </div>             
            </div>
          </div>

          <div class="list-new-product d-none">
            <h2 class="list-new-product-title fw-700 text-center mb-3">New Product</h2>
            <div class="row">
              <div class="col-xl-4 col-md-4 col-sm-12 col-12 mt-3">
                <div class="card border-grey-color">
                  <img src="./assets/images/new_product_1.svg" class="card-img-top position-relative" alt="">
                  <img src="./assets/images/icon/news.svg" class="icon-new position-absolute" alt="" style="top: 15px; left: 16px;">
                  <div class="card-body">
                    <span class="date-time fw-500 font-size-14 text-grey-color">27/05/2022</span>
                    <h5 class="card-title limit-line-2 fw-700 font-size-18 mb-2">Lorem Ipsum is simply dummy text of the printing.</h5>
                    <p class="card-text limit-line-2 fw-400 light-black-color pr-2">Lorem Ipsum is simply dummy text of the printing and
                      typesetting
                      industry.</p>
                  </div>
                </div>
              </div>
              <div class="col-xl-4 col-md-4 col-sm-12 col-12 mt-3">
                <div class="card border-grey-color">
                  <img src="./assets/images/new_product_2.svg" class="card-img-top position-relative" alt="">
                  <img src="./assets/images/icon/news.svg" class="icon-new position-absolute" alt="" style="top: 15px; left: 16px;">
                  <div class="card-body">
                    <span class="date-time fw-500 font-size-14 text-grey-color">27/05/2022</span>
                    <h5 class="card-title limit-line-2 fw-700 font-size-18 mb-2">Lorem Ipsum is simply dummy text of the printing.</h5>
                    <p class="card-text limit-line-2 fw-400 light-black-color pr-2">Lorem Ipsum is simply dummy text of the printing and
                      typesetting
                      industry.</p>
                  </div>
                </div>
              </div>
              <div class="col-xl-4 col-md-4 col-sm-12 col-12 mt-3">
                <div class="card border-grey-color">
                  <img src="./assets/images/new_product_3.svg" class="card-img-top position-relative" alt="">
                  <img src="./assets/images/icon/news.svg" class="icon-new position-absolute" alt="" style="top: 15px; left: 16px;">
                  <div class="card-body">
                    <span class="date-time fw-500 font-size-14 text-grey-color">27/05/2022</span>
                    <h5 class="card-title limit-line-2 fw-700 font-size-18 mb-2">Lorem Ipsum is simply dummy text of the printing.</h5>
                    <p class="card-text limit-line-2 fw-400 light-black-color pr-2">Lorem Ipsum is simply dummy text of the printing and
                      typesetting
                      industry.</p>
                  </div>
                </div>
              </div>



            </div>
          </div>
        </div>

    </section>
  </main>
  <?php include 'footer.php'; ?>



  <!-- SCRIPT -->
  <!-- CDN -->
  <!-- jQuery first, then Popper.js, then Bootstrap JS -->
  <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
  <script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
  <script src="./assets/js/home.js"></script>
  <script>
    $('#product').addClass('border-text-color')
  </script>
</body>

</html>